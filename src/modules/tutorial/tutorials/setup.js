export default async () => {
  ...
  let tutorialStep = game.settings.get("vtta-ddb", "tutorial-step");
  ...
  if (
    tutorialStep > 0 &&
    game.user.data._id !== game.settings.get("vtta-ddb", "tutorial-user")
  )
    return;
  let hint;
  if (tutorialStep === 0) {
  ...
  if (tutorialStep === 2) {
    ...
    hint = await window.vtta.hint.show(
         ...
         `,
      {
        ...
        hide: {
          selector:
            '#sidebar section[data-tab="compendium"] footer.directory-footer button[type="submit"]',
          event: "click",
        },
        width: 400,
      }
